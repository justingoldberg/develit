...
function formRelatorioBaixaGrupoServico($modulo, $sub, $acao, $registro, $matriz) {
	...
	if(!$permissao[admin]) {
		...
		novaTabela2("[Baixas por Grupos de Serviços]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 2);
			# Opcoes Adicionais
...
//			$matriz[dtFinal]=substr($matriz[dtFinal],0,2)."/".substr($matriz[dtFinal],2,4);
		...
		*/
		...
		$matCabecalho=array(  "Grupo", "Cliente", "Baixa",  "Valor", "Opções");
...
//				} 
			...
			if (count($matResultado) > 0){
				...
				$matrizRelatorio[header][TITULO]="BAIXAS POR GRUPOS DE SERVIÇOS";
