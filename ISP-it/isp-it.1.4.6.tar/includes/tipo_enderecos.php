		...
		novaTabela2("[Cadastro de Tipos de Enderecos]", "center", '100%', 0, 3, 1, $corFundo, $corBorda, 4);
			novaLinhaTabela($corFundo, '100%');
				htmlAbreColuna('65%', 'left', $corFundo, 0, 'tabfundo1');
					echo "<br><img src=".$html[imagem][enderecos]." border=0 align=left><b class=bold>Tipos de Endereços</b>
					<br><span class=normal10>Cadastro de <b>tipos de endereços</b>, utilizados para o cadastro de pessoas.</span>";
				htmlFechaColuna();			
	...
	elseif($matriz[bntAdicionar]) {
		...
		if($matriz[descricao] && $matriz[valor]) {
			...
			if(contaConsulta(buscaTipoEnderecos($matriz[valor], 'valor', 'igual','valor'))>0){
				# Erro - campo inválido
				# Mensagem de aviso
				$msg="Tipo de Endereço já cadastrado!";
...
function listarTipoEnderecos($modulo, $sub, $acao, $registro, $matriz)
{
	...
	novaTabela("[Listar]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 3);
		...
		if(!$consulta || contaConsulta($consulta)==0) {
			...
			# Cabeçalho
			novaLinhaTabela($corFundo, '100%');
				itemLinhaTabela('Tipo de Endereço', 'center', '60%', 'tabfundo0');
...
function formSelectTipoEndereco($tipoEndereco, $campo, $tipo) {
	...
	if($consulta && contaConsulta($consulta)>0) {
		...
		elseif($tipo=='check') {
			...
			else $retorno='Tipo de Endereço inválido!';
		}
	}
