...
function tipoCobranca($modulo, $sub, $acao, $registro, $matriz)
{
	...
	if(!$permissao[admin]) {
		...
		novaTabela2("[Cadastro de Tipos de Cobrança]", "center", '100%', 0, 3, 1, $corFundo, $corBorda, 4);
			novaLinhaTabela($corFundo, '100%');
				htmlAbreColuna('65%', 'left', $corFundo, 0, 'tabfundo1');
					echo "<br><img src=".$html[imagem][cobrancas]." border=0 align=left><b class=bold>Tipos de Cobrança</b>
					<br><span class=normal10>Cadastro de <b>tipos de cobrança</b>, utilizados para o cadastro de serviços.</span>";
				htmlFechaColuna();			
		...
		novaTabela2("[Adicionar]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 2);
			...
			novaLinhaTabela($corFundo, '100%');
				htmlAbreColuna('40%', 'right', $corFundo, 0, 'tabfundo1');
					...
					<span class=normal10>Descrição do tipo de Cobrança</span>";
				htmlFechaColuna();
			...
			novaLinhaTabela($corFundo, '100%');
				htmlAbreColuna('40%', 'right', $corFundo, 0, 'tabfundo1');
					...
					<span class=normal10>Cobrança proporcional ao número de dias</span>";
				htmlFechaColuna();
				itemLinhaForm(formSelectSimNao($matriz[valor],'valor','form'), 'left', 'top', $corFundo, 0, 'tabfundo1');
			fechaLinhaTabela();
			novaLinhaTabela($corFundo, '100%');
				htmlAbreColuna('40%', 'right', $corFundo, 0, 'tabfundo1');
					echo "<b class=bold>Forma Cobrança: </b><br>
					<span class=normal10>Forma de geração da cobrança</span>";
				htmlFechaColuna();
			...
			novaLinhaTabela($corFundo, '100%');
				htmlAbreColuna('40%', 'right', $corFundo, 0, 'tabfundo1');
					...
					<span class=normal10>Tipo de Cobrança</span>";
				htmlFechaColuna();
	...
	elseif($matriz[bntAdicionar]) {
		...
		if($matriz[descricao] && $matriz[valor] && $matriz[forma] && $matriz[tipo]) {
			...
			if(contaConsulta(buscaTipoCobranca($matriz[descricao], 'descricao', 'igual','descricao'))>0){
				# Erro - campo inválido
				# Mensagem de aviso
				$msg="Tipo de Cobrança já cadastrado!";
...
function listarTipoCobranca($modulo, $sub, $acao, $registro, $matriz)
{
	...
	novaTabela("[Listar]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 5);
		...
		if(!$consulta || contaConsulta($consulta)==0) {
			...
			# Cabeçalho
			novaLinhaTabela($corFundo, '100%');
				itemLinhaTabela('Tipo de Cobrança', 'center', '40%', 'tabfundo0');
				itemLinhaTabela('Proporcional', 'center', '10%', 'tabfundo0');
				itemLinhaTabela('Forma Cobrança', 'center', '10%', 'tabfundo0');
				itemLinhaTabela('Tipo Cobrança', 'center', '20%', 'tabfundo0');
