...
function validacao($sessLogin, $modulo, $sub, $acao, $registro) {
	...
	global $configAppName, $configAppVersion, $corFundo, $corBorda, $sessLogin, $html;
	
	# Formulario de login
	if(!$sessLogin[login] || !$sessLogin[senha]) {
		# Formulário de login
		
		if(!$modulo || $modulo=='logoff' || $modulo=='login') {
			$modulo='';
			$sub='';
			$acao='';
			$registro='';
			...
			novaLinhaTabela($corFundo, '100%');
				...
				htmlAbreColuna('40%', 'right', $corFundo, 0, 'normal10');
					novaTabela2("[Login de Acesso]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 2);
						...
						novaLinhaTabela($corFundo, '100%');
							$texto="<form method=post name=matriz action=$_SERVER[PHP_SELF]>
							...
							<br>
							<span class=txtaviso>Informe Usuário e Senha de Acesso</span></center><br>";
							itemLinhaNOURL($texto, 'left', $corFundo, 2, 'tabfundo1');
						fechaLinhaTabela();
						novaLinhaTabela($corFundo, '100%');
							htmlAbreColuna('40%', 'right', $corFundo, 0, 'tabfundo1');
								echo "<b class=bold10>Usuário: </b>";
							htmlFechaColuna();
...
# Função para checagem de formulário de validação
...
} #fecha validação de formulário de login



# Função para checagem de login
 ...
 * @param unknown $tipo
 * @param unknown $ordem
 * @desc Retorna um ResultSet com os registros de usuarios.
*/
...
# Buscar infomações do grupo e retornar matriz
function buscaInfoGrupo($idGrupo) {
	...
	return($infoGrupo);
} #fecha funcao de informações do grupo



# Função para busca de ID de usuario
 ...
 * @param unknown $tipo
 * @param unknown $ordem
 * @desc Retorna o LOGIN do usuario informado em TEXTO
*/
...
function buscaPermissaoUsuario($login) {
	...
	# Checar informação sobre usuarios - localizar grupos
	...
	if($consulta && contaConsulta($consulta)>0) {
		...
		while($i < contaConsulta($consulta)) {
			...
			# Buscar informações sobre este grupo
			$consultaGrupo=buscaGrupos($idGrupo, 'id','igual','id');
			...
			$i++;
		} #fecha atualização de permissões
	}
			...
			novaTabela2("[Acesso]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 4);
				novaLinhaTabela($corFundo, '100%');
					htmlAbreColuna('55%', 'left', $corFundo, 0, 'tabfundo1');
						...
						<br><span class=normal10>Utilize a seção de Acesso para controlar
...
function cadastroUsuarios($modulo, $sub, $acao, $registro, $matriz)
{
	...
	if(!$permissao[admin]) {
		...
		novaTabela2("[Cadastro de Usuários]", "center", '100%', 0, 3, 1, $corFundo, $corBorda, 4);
			novaLinhaTabela($corFundo, '100%');
				htmlAbreColuna('65%', 'left', $corFundo, 0, 'tabfundo1');
					echo "<br><img src=".$html[imagem][cadastro]." border=0 align=left><b class=bold>Usuarios</b>
					<br><span class=normal10>O cadastro de usuários provê o controle de usuários que terão
...
function verStatusUsuarios()
{
	...
	# Motrar tabela de busca
	novaTabela2("[Informações sobre Cadastro de Usuarios]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 3);
		novaLinhaTabela($corFundo, '100%');
			htmlAbreColuna('60%', 'left', $corFundo, 0, 'tabfundo1');
			echo "<br><img src=".$html[imagem][status]." border=0 align=left><b class=bold>Status dos Usuários</b><br>
			<span class=normal10>Status e informações sobre o cadastro de usuários.";
			htmlFechaColuna();
		...
		novaTabela2("[Adicionar]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 2);
			...
			novaLinhaTabela($corFundo, '100%');
				htmlAbreColuna('40%', 'right', $corFundo, 0, 'tabfundo1');
					...
					<span class=normal10>Confirmação de Senha de acesso do usuário</span>";
				htmlFechaColuna();
...
function listarUsuarios($modulo, $sub, $acao, $registro, $matriz) {
	...
	# Cabeçalho		
	# Cabeçalho
	$largura 				= array( '40%',		   '20%',		'40%' );
	$gravata['cabecalho']   = array( 'Login',	'Status',	 'Opções' );
	$gravata['alinhamento'] = array( 'center', 	'center',	 'center' );
	...
	novaTabela("[Listar]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 3);
		...
		$usuarios = getArrayObjetos( $consulta );
		...
		if(!$totalUsuarios) {
			...
			# Paginador
			paginador( $consulta, $totalUsuarios, $limite['lista']['usuarios'], $registro, 'normal', $qtdColunas, '' );
			...
			while($i < $totalUsuarios && $i < $limite) {
				# Mostrar registro
