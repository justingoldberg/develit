...
#
# Função:
#    Painel - Funções para gerenciamento de aplicacao


function aplicacao($modulo, $sub, $acao, $registro, $matriz) {
	...
	if(!$permissao[admin] && !$permissao[adicionar]) {
		...
		if($acao=="adicionar") {
			echo "<br>";
			aplicacaoAdicionar($modulo, $sub, $acao, $registro, $matriz);
		}
		elseif($acao=="alterar") {
			echo "<br>";
			aplicacaoAlterar($modulo, $sub, $acao, $registro, $matriz);
		}
		elseif($acao=='procurar') {
			echo "<br>";
			aplicacaoProcurar($modulo, $sub, $acao, $registro, $matriz);
		}
		elseif($acao=='ver') {
			echo "<br>";
			aplicacaoVer($modulo, $sub, $acao, $registro, $matriz);
		}
		elseif($acao=='excluir') {
			echo "<br>";
			aplicacaoExcluir($modulo, $sub, $acao, $registro, $matriz);
		}
		elseif($acao=='listar') {
			echo "<br>";
			aplicacaoListar($modulo, $sub, $acao, $registro, $matriz);
		}
		/*
 ...
 * @desc Funcao para exclusao de aplicacoes adicionadas às Ordem de Serviço
*/
function aplicacaoExcluir($modulo, $sub, $acao, $registro, $matriz) {
	/*$msg="Esta opção não está habilitada";
	avisoNOURL("Aviso: Exclusão", $msg, 400);
	echo "<br>";
	aplicacaoVer($modulo, $sub, 'ver', $registro, $matriz);*/
		/*$msg="Esta opção não está habilitada";
	avisoNOURL("Aviso: Exclusão", $msg, 400);
	echo "<br>";
	...
	elseif($matriz[bntExcluir]) {
		...
		if($matriz[id]) {
			...
			$grava=dbAplicacao($matriz, 'excluir');
			...
			if($grava) {
				...
				echo "<br>";
				aplicacaoVer($modulo, $sub, 'ver', "$matriz[id]", $matriz);
			} else {
				echo "<br>";
...
} // fim da funcao de exclusao de Aplicacao



function aplicacaoProcurar($modulo, $sub, $acao, $registro, $matriz) {
	...
	if( !$matriz[bntProcurar] ) {
		...
		# realizar consulta
		$lista=buscaRegistros($matriz[nome], 'descricao', 'contem', 'descricao', $tb[Aplicacao]);
		aplicacaoListar($modulo, $sub, 'ver', $lista, $matriz);
	}
}

# função para adicionar
function aplicacaoAdicionar($modulo, $sub, $acao, $registro, $matriz) {
	...
	if(!$matriz[bntAdicionar]) {
		...
		novaTabela2("[Adicionar]<a name=ancora></a>", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 2);
			...
			$dados=dadosAplicacao(0);
			
			aplicacaoMostra($dados);
		...
		//$matriz[id]=buscaNovoID($tb[Aplicacao]);
		$grava=dbAplicacao($matriz, 'incluir');
		...
		if($grava) {
			...
			avisoNOURL("Aviso: ", $msg, 400);
			echo "<br>";
			aplicacaoListar($modulo, $sub, 'listar', $matriz[id], $matriz);
		} 
		else {
			$msg="Ocorreram erros durante a gravação.";
			avisoNOURL("Aviso: Ocorrência de erro", $msg, 400);
...
function aplicacaoAlterar($modulo, $sub, $acao, $registro, $matriz) {
	...
	if($registro && !$matriz[bntConfirmar]) {
		...
		# Buscar Valores
		$dados=dadosAplicacao($registro, 'id', 'igual', 'id');
	...
	elseif($matriz[bntConfirmar]) {
		...
		if($matriz[id]) {
			# continuar
			# Cadastrar em banco de dados
			$grava=dbAplicacao($matriz, 'alterar');
...
function aplicacaoMostra($dados) {
...
function aplicacaoBuscaIDNovo() {

	global $conn, $tb;
	
	$sql="SELECT count(id) qtde from $tb[Aplicacao]";
	$consulta=consultaSQL($sql, $conn);
	
	if($consulta && resultadoSQL($consulta, 0, 'qtde')>0) {
	
		$sql="SELECT MAX(id)+1 id from $tb[Aplicacao]";
		$consulta=consultaSQL($sql, $conn);
...
function aplicacaoListar($modulo, $sub, $acao, $lista, $matriz) {
	...
	if(!$permissao[admin] && !$permissao[visualizar]) {
		...
		if (! $lista) 
			$consulta=buscaRegistros('status <> "I"', 'status', 'custom', 'descricao', $tb[Aplicacao]);
		else
			$consulta=$lista;
...
# função Exibição
function aplicacaoVer($modulo, $sub, $acao, $registro, $matriz) {
	...
	if(!$permissao[admin] && !$permissao[visualizar]) {
		...
		# Procurar dados3
		$objeto=dadosAplicacao($registro);
 ...
 * @desc Retorna 1 ou varios itens do objeto.
 item  = id do aplicacao
 campo = campo da matriz
 retorno = campo de retorno (nome do componente)
 tipo = check - retorna a descricao
        form  - um combo com as descricoes
        multi - uma caixa de selecao com as descricoes
*/
function formSelectAplicacao($item, $campo, $retorno, $tipo) {
...
# Função para Dados
function dadosAplicacao($id) {

	global $tb;

	$consulta=buscaRegistros($id, 'id', 'igual', 'id', $tb[Aplicacao]);
...
function dbAplicacao($matriz, $tipo) {
