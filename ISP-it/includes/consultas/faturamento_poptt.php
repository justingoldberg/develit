	...
	if($consultaPOP && contaConsulta($consultaPOP)>0) {
		...
		for($x=0;$x<contaConsulta($consultaPOP);$x++) {
			...
			if(is_array($total)) {
				...
				# Cabeçalho
				$matResultado=array();
				$matCabecalho=array($grvMes, "Faturado", "Liquido", "Juros", "Descontos", "Bruto", "Pendente");
				...
				novaLinhaTabela($corFundo, '100%');
					...
					itemLinhaTabela('Faturado no Mês', 'center', '10%', 'tabfundo0');
					...
					itemLinhaTabela('Pendente (Fat-Liq)', 'center', '10%', 'tabfundo0');
				fechaLinhaTabela();
				...
				if($x+1<contaConsulta($consultaPOP)) itemTabelaNOURL('&nbsp;','left',$corFundo,8,'normal10');
		...
		if(is_array($matrizRelatorio) && count($matrizRelatorio)>0) {
			itemTabelaNOURL('&nbsp;','left',$corFundo,8,'normal10');
	...
	if($consultaPOP && contaConsulta($consultaPOP)>0) {
		...
		for($x=0;$x<contaConsulta($consultaPOP);$x++) {
			...
			if(is_array($matTotal)) {
				...
				if($x+1<contaConsulta($consultaPOP)) itemTabelaNOURL('&nbsp;','left',$corFundo,7,'normal10');
