...
function dbParametro($matriz, $tipo)
{
	...
	$matriz[valor]=base64_encode($matriz[valor]);
...
function listarParametros($modulo, $sub, $acao, $registro, $matriz)
{
	...
	novaTabela("[Listar]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 3);
		...
		if(!$consulta || contaConsulta($consulta)==0) {
			...
			# Paginador
			$acao='listar';
			paginador($consulta, contaConsulta($consulta), $limite[lista][parametros], $registro, 'normal10', 3, $urlADD);
...
function procurarParametros($modulo, $sub, $acao, $registro, $matriz)
{
	...
	if($matriz[txtProcurar] && $matriz[bntProcurar]) {
		#buscar registros
		$consulta=buscaParametros("upper(descricao) like '%$matriz[txtProcurar]%' OR upper(parametro) like '%$matriz[txtProcurar]%' OR upper(valor) like '%$matriz[txtProcurar]%'",$campo, 'custom','descricao');
		...
		elseif($consulta && contaConsulta($consulta)>0 && (!$registro || is_numeric($registro)) ) {	
			...
			# Paginador
			paginador($consulta, contaConsulta($consulta), $limite[lista][parametros], $registro, 'normal10', 4, $urlADD);
...
function alterarParametros($modulo, $sub, $acao, $registro, $matriz)
{
	...
	elseif($registro && !$matriz[bntAlterar]) {
		...
		if(!$consulta || contaConsulta($consulta)==0) {
			...
			$valor=base64_decode(resultadoSQL($consulta, 0, 'valor'));
	...
	elseif($matriz[bntAlterar]) {
		...
		if($matriz[descricao] && $matriz[parametro] && $matriz[valor]) {
			...
			if($grava) {
				...
				echo "<br>";
				listarParametros($modulo, $sub, 'listar', 0, $matriz);
...
function excluirParametros($modulo, $sub, $acao, $registro, $matriz)
{
		...
		if(!$consulta || contaConsulta($consulta)==0) {
			...
			novaTabela2("[Excluir]", "center", '100%', 0, 2, 1, $corFundo, $corBorda, 2);
				...
				novaLinhaTabela($corFundo, '100%');
					$texto="<b class=bold>Valor: </b>";
					itemLinhaTMNOURL($texto, 'right', 'middle', '40%', $corFundo, 0, 'tabfundo1');
					itemLinhaTMNOURL(base64_decode($valor), 'left', 'middle', '60%', $corFundo, 0, 'tabfundo1');
				fechaLinhaTabela();
				novaLinhaTabela($corFundo, '100%');
	...
	elseif($matriz[bntExcluir]) {
		...
		if($grava) {
			...
			echo "<br>";
			listarParametros($modulo, $sub, $acao, 0, $matriz);
		}
...
function carregaParametros() {
	...
	$consulta=buscaParametros('','','todos','descricao');
	...
	if($consulta && contaConsulta($consulta)>0) {
		...
		for($a=0;$a<contaConsulta($consulta);$a++) {
			...
			$parametros[$parametro]=base64_decode($valor);
		}
	}
